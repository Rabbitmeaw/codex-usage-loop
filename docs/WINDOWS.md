...
src/CodexUsageLoop.Core/
  ...
  UsageRingStyle.cs          圆环描边与 DPI 度量
...
6. 每秒读取一次 pet 布局和鼠标位置。只有几何、额度、颜色或动画帧变化时
   ...
   立即停表。用量每 30 秒主动刷新，并同时接受 app-server 推送。桌面
   悬浮圆环使用 B28 整数描边的 85%，并在目标显示器物理像素阶段再次
   四舍五入：100% DPI 下标准围绕圆环为 7 px，240 DPI 下为 16 px；
   32×32 通知区域图标保留独立固定描边。
7. 用量卡片以 96 DPI 下的 190×58/74 DIP 为基准，窗口尺寸、字体、圆点、
   ...
   同时提高深色背景下的可读性。卡片底部额外保留 4 DIP 留白；状态文字
   和上方内容不移动，围绕 pet 时卡片顶部锚点也保持不变。
...
等比换算），且使用较紧凑的
固定估算直径。Codex 状态提供显式 mascot 矩形时，既有位置和直径计算不变。默认
100% 可直接使用；若估算来源下的视觉直径不合适，可选地从菜单的 **围绕 pet 的圆环大小**
选择预设：pet 在环内显得太小时选更小的 50% 或 25%，圆环太紧时选更大的 150%
或 250%。选择会保存，且只改变圆环，不会改变 pet 本身。
...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass `
  -File scripts/package-windows.ps1 -Version 0.1.3
```
...
- 不做逐像素窗口捕获；当 Codex 状态仅有锚点或状态不可读时，位置和尺寸属于
  估算，不保证与实际 pet 边界完全一致。默认 100% 仍可直接使用；若希望更贴近
  自己的视觉偏好，可选地用“围绕 pet 的圆环大小”选择更小或更大的预设。
