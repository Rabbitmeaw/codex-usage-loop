...
final class PetWindowLocator {
    ...
    func locate() -> PetWindow? {
        ...
        let overlayOrigin = codexOverlayOrigin()
        let placement = codexPlacement()
    ...
    private func codexPlacement() -> String? {
        let path = NSHomeDirectory() + "/.codex/.codex-global-state.json"
        guard let data = FileManager.default.contents(atPath: path),
              let root = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let bounds = root["electron-avatar-overlay-bounds"] as? [String: Any] else { return nil }
        return bounds["placement"] as? String
    }

    private func codexOverlayOrigin() -> CGPoint? {
        let path = NSHomeDirectory() + "/.codex/.codex-global-state.json"
        guard let data = FileManager.default.contents(atPath: path),
              let root = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let bounds = root["electron-avatar-overlay-bounds"] as? [String: Any],
              let x = number(bounds["x"]), let y = number(bounds["y"]) else { return nil }
